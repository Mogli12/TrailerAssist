...
function trailerAssist.globalsReset( createIfMissing )
	...
	file = getUserProfileAppPath().. "modsSettings/FS19_TrailerAssist/config.xml"
	if fileExists(file) then	
		print('Loading "'..file..'"...')
		trailerAssist.globalsLoad( file, "trailerAssistGlobals", trailerAssistGlobals )	
	end
...
function trailerAssist:onUpdate(dt)
...
function trailerAssist:onRegisterActionEvents(isSelected, isOnActiveVehicle)
	if self.isClient and isOnActiveVehicle then
		...
		for _,actionName in pairs({ "taMODE",  
			...
			local _, eventName = self:addActionEvent(self.taActionEvents, InputAction[actionName], self, trailerAssist.actionCallback, pBool1, pBool2, pBool3, pBool4, nil);
...
-- updateTick 
--***************************************************************
function trailerAssist:onUpdateTick(dt)
...
function trailerAssist:getMovingDirection()
	...
	if self.vcaMovingDir ~= nil then 
		return self.vcaMovingDir
